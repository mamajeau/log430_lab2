...
public class SimulatedBank {
   ...
   */
  private Status completeDeposit(Message message, Transaction currTransact) {
   ...
   */
  public Status handleMessage(Message message, Transaction currTransact) {
    int cardNumber = message.getCard().getNumber();
    ...
    case Message.WITHDRAWAL:
      return withdrawal(message, currTransact);
    case Message.INITIATE_DEPOSIT:
      return initiateDeposit(message);
    case Message.COMPLETE_DEPOSIT:
      return completeDeposit(message, currTransact);
    case Message.TRANSFER:
      return transfer(message, currTransact);
    case Message.INQUIRY:
      return inquiry(message, currTransact);
    }
   ...
   * @return status code derived from current values
   */
  private Status inquiry(Message message, Transaction currTransact) {
    int cardNumber = message.getCard().getNumber();
   ...
   * @return status code derived from current values
   */
  private Status transfer(Message message, Transaction currTransact) {
    int cardNumber = message.getCard().getNumber();
   ...
   * @return status code derived from current values
   */
  private Status withdrawal(Message message, Transaction currTransact) {
    int cardNumber = message.getCard().getNumber();
    int accountNumber = ACCOUNT_NUMBER[cardNumber][message.getFromAccount()];
    if (accountNumber == 0) {
      return new Failure("Invalid account type");
