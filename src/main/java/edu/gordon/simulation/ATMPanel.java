/* ATM Example system - file ATMPanel.java copyright (c) 2001 - Russell C. Bjork */
...
class ATMPanel extends Panel {
   ...
   *
   */
  private static final long serialVersionUID = -7704596260440179390L;
  ...
  private static final int OPERATOR_COL = SHOW_LOG_BUTTON_COL
      + SHOW_LOG_BUTTON_WIDTH;
   ...
   *          the the overall GUI
   * @param operatorPanel
   *          the edu.gordon.simulation of the card reader
   * @param cardReader
   *          the edu.gordon.simulation of the card reader
   * @param display
   *          the edu.gordon.simulation of the display
   * @param keyboard
   *          the edu.gordon.simulation of the keyboard
   * @param cashDispenser
   *          the edu.gordon.simulation of the cash dispenser
   * @param receiptPrinter
   *          the edu.gordon.simulation of the receipt printer
   */
  ATMPanel(final GUI gui, SimOperatorPanel operatorPanel,
      SimCardReader cardReader, SimCustomerConsole display,
      SimKeyboard keyboard, SimCashDispenser cashDispenser,
      SimEnvelopeAcceptor envelopeAcceptor, SimReceiptPrinter receiptPrinter) {
    GridBagLayout atmLayout = new GridBagLayout();
    setLayout(atmLayout);
    // Add the operator panel
    add(operatorPanel);
    atmLayout.setConstraints(operatorPanel, GUI.makeConstraints(OPERATOR_ROW,
        OPERATOR_COL, OPERATOR_WIDTH, OPERATOR_HEIGHT, OPERATOR_FILL));
    ...
    add(cardReaderPanel);
    atmLayout.setConstraints(cardReaderPanel, GUI.makeConstraints(READER_ROW,
        READER_COL, READER_WIDTH, READER_HEIGHT, READER_FILL));
    ...
    add(display);
    atmLayout.setConstraints(display, GUI.makeConstraints(DISPLAY_ROW,
        DISPLAY_COL, DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_FILL));
    add(keyboard);
    atmLayout.setConstraints(keyboard, GUI.makeConstraints(KEYBOARD_ROW,
        KEYBOARD_COL, KEYBOARD_WIDTH, KEYBOARD_HEIGHT, KEYBOARD_FILL));
    // Add the cash dispenser
    add(cashDispenser);
    atmLayout.setConstraints(cashDispenser, GUI.makeConstraints(DISPENSER_ROW,
        DISPENSER_COL, DISPENSER_WIDTH, DISPENSER_HEIGHT, DISPENSER_FILL));
    ...
    atmLayout.setConstraints(envelopeAcceptorPanel,
        GUI.makeConstraints(ENVELOPE_ROW, ENVELOPE_COL, ENVELOPE_WIDTH,
            ENVELOPE_HEIGHT, ENVELOPE_FILL));
    envelopeAcceptor.setVisible(false);
    // Add the receipt printer
    add(receiptPrinter);
    atmLayout.setConstraints(receiptPrinter, GUI.makeConstraints(PRINTER_ROW,
        PRINTER_COL, PRINTER_WIDTH, PRINTER_HEIGHT, PRINTER_FILL));
