...
public class Session {
   ...
   */
  public Status performInvalidPINExtension(Message message,
    ...
    for (int i = 0; i < 3; i++) {
      ...
      message.setPIN(pin);
      status = atm.getNetworkToBank().sendMessage(message, currentTransaction);
  ...
  public boolean performTransaction(Transaction currentTransaction)
    ...
    while (true) // Terminates by return in ASKING_DO_ANOTHER_STATE or exception
    {
      ...
      case Transaction.SENDING_TO_BANK_STATE:
        status = atm.getNetworkToBank().sendMessage(message,
            currentTransaction);
        if (status.isInvalidPIN()) {
      ...
      case Transaction.INVALID_PIN_STATE:
        try {
          status = performInvalidPINExtension(message, currentTransaction);
